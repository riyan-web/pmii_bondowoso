        ...
        <div class="row no-gutters">
            ...
            <div class="col-md-8">
                <div class="card-body">
                    ...
                    <h4 class="card-title">Deskripsi Cabang : </h4>
                    <p class="card-text"><?php echo $komisariat['isi']; ?></p>
                    <button class="col-md-4 btn btn-sm btn-warning" title="Ubah" onclick="komisariat_ubah(<?php echo $komisariat['id']; ?>)"><i class="fa fa-edit"></i> Ubah Profile Cabang</button>
                </div>
            </div>
        </div>
    </div>
...
<script>
    function komisariat_ubah(id) {
        ...
        $.ajax({
            url: "<?= site_url('admin/profile_cabang/komisariat_ubah') ?>/" + id,
            type: "GET",
            dataType: "JSON",
    ...
    function simpan() {
        ...
        if (save_method == 'tambahKomisariat') {
            url = "<?= site_url('admin/profile_cabang/komisariat_tambah') ?>";
        } else {
            url = "<?= site_url('admin/profile_cabang/komisariat_proses_ubah') ?>";
        }
